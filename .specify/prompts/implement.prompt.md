...
*   **Bug Fixes Add Structure:** When fixing a bug, first add (or sharpen) the requirement — normally an EARS Unwanted Behavior line `If <condition>, then the system shall <response>` — with a new ID, then write a test titled with that ID that reproduces the failure, then fix the code and tag it. Commit the fix with a `fix(scope): … [ID]` subject; CI rejects a `fix` whose change set adds no spec line and no test for the cited ID.
*   **Reference IDs in Commits:** Any commit that changes `src/` must cite the affected `[REQ-…]`/`[ARC-…]` IDs in its message (or `[NO-REQ]` with a reason); the `commit-msg` hook and CI enforce this.
